...
Uma vez analisados os protocolos de aplicação e frameworks existentes (seção \ref{sec:relwork}), bem como os protocolos de rede (seção \ref{sec:commprot}), e tendo em vista os requisitos levantados para a rede de sensores (seções \ref{sec:reqfunc} e \ref{sec:reqnfunc}), pode-se passar para a especificação do protocolo de aplicação proposto, batizado de \textit{Rainfall}. Os requisitos relativos ao protocolo estão reescritos abaixo, por fins de claridade.
...
\clearpage
\section{Implementação de Bibliotecas para o Protocolo de Aplicação}

...
Para tanto, é importante estudar trabalhos relacionados e levantar as características dos protocolos de comunicação subjacentes existentes, correspondentes às camadas de rede de nível inferior, sobre as quais o protocolo de aplicação a ser desenvolvido será executado.

\section{Trabalhos relacionados}\label{sec:relwork}
\paragraph{IoTivity.} O IoTivity \cite{iotivity} é um projeto colaborativo da fundação Linux que tem por objetivo prover um padrão para prover conectividade entre dispositivos no contexto de internet das coisas. Suas funcionalidades incluem prover descoberta de dispositivos, transmissão de dados, gerenciamento de dispositivos e gerenciamento de dados, como ilustra a \ref{fig:iotivity}. Seu funcionamento se baseia em uma arquitetura REST, por meio da qual dados e mensagens de controle são trafegadas. 
...
A biblioteca desenvolvida possuirá uma biblioteca dividida em camadas, conforme ilustra a Figura \ref{fig:libarchutecture}. As camadas apresentadas em cor verde compõem a biblioteca a ser desenvolvida no projeto.

%\begin{figure}[h]
%	\centering
%	\caption{Arquitetura em camadas da biblioteca}
%	\includegraphics[width=0.8\textwidth]{imagens/zwavelayers.jpg}
% 	\label{fig:libarchutecture}
%\end{figure}

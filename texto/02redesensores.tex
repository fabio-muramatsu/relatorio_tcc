...
Os requisitos definidos na seção \ref{subsec:requisitos} permitem definir as chaves e valores associados a serem utilizados no protocolo, conforme mostra a Tabela \ref{tab:chavevalorprot}. A descrição desses campos estão apresentados a seguir.
...
\paragraph*{\texttt{packageType.}} Descreve o tipo do pacote, definindo o significado dos demais campos presentes no pacote. Possíveis valores incluem:
\begin{itemize}
	\item \texttt{whoiscontroller}: Esta mensagem é enviada por um dispositivo sensor ou atuador  por broadcast para descobrir o endereço do controlador;
	\item \texttt{iamcontroller}: Mensagem enviada em resposta ao pedido de descoberta de controlador;
	\item \texttt{describeyourself}: Mensagem enviada pelo controlador solicitando ao dispositivo-alvo uma descrição de suas funcionalidades;
	\item \texttt{description}: Consiste na descrição das funcionalidades do dispositivo, enviada em resposta ao pedido \texttt{describeyourself};
	\item \texttt{data}: Descreve uma mensagem contendo dados de leitura de um sensor;
	\item \texttt{command}: Descreve uma mensagem contendo comandos destinados a um atuador;
	\item \texttt{lifetime}: Mensagem especificando o intervalo máximo de envio de sinais \textit{watchdog};
	\item \texttt{keepalive}: Especifica uma mensagem que deve ser interpretada como sinal \textit{watchdog}.
\end{itemize} 

\paragraph*{\texttt{nodeClass}.} Define a classe de um dispositivo, que pode ser sensor, atuador ou controlador. Um sensor é definido como um dispositivo que envia dados de leitura. Um controlador é um dispositivo que recebe comandos e efetua ações baseadas neles. Um controlador é um dispositivo que coordena sensores e atuadores, recebendo dados de leitura e enviando comandos de atuação.

\paragraph*{\texttt{data}.} Especifica o dado lido por um sensor. O formato do valor associado depende do tipo de dados declarado na mensagem do tipo \texttt{description}.

\paragraph*{\texttt{command}.} Especifica o comando destinado a um atuador. O formado do valor associado depende do tipo de comandos aceitos declarado na mensagem do tipo \texttt{description}.

\paragraph*{\texttt{lifetime}.} Especifica o intervalo de tempo máximo entre mensagens de \textit{watchdog}. Esta mensagem é emitida pelo controlador aos dispositivos, regulando a frequência de envio de mensagens do tipo \texttt{keepalive}.

\paragraph*{\texttt{yourId}.} Esta mensagem é emitida pelo controlador, definindo o identificador a ser utilizado pelos dispositivos da rede nas comunicações subsequentes.

\paragraph*{\texttt{dataType}.} Especifica as classes de dados que são emitidos pelo sensor. Consiste em um objeto com chaves e valores definidos conforme mostra a Tabela \ref{tab:datatype}. O campo \texttt{category} define a categoria de dados coletados pelo sensor, além de especificar os tipos que podem ser alocados no campo \texttt{type}. A Tabela \ref{tab:classesdados} define os valores possíveis de serem utilizados como categoria, além de listar os tipos de dados permitidos para cada categoria.

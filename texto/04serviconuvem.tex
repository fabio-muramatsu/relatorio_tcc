...
A Figura \ref{fig:mensagens_accept_node} mostra as mensagens trocadas neste processo. O servidor inicia informando ao usuário (aplicativo) que existe um novo nó com aceitação pendente através da mensagem \texttt{Notify Detected Node}. O usuário, então, responde com uma mensagem \texttt{Accept Node}, informando sua decisão ao servidor. Este, por fim, envia uma notificação \texttt{Notify Accepted Node} ao controlador, informando-o da decisão do usuário.
...
A Figura \ref{fig:mensagens_new_rule} mostra as mensagens trocadas neste processo. O servidor inicia informando ao usuário (aplicativo) que existem novas regras para análise através da mensagem \texttt{Notify Learnt Rules}. O usuário, então, envia uma mensagem \texttt{Get Rules}, para que o servidor lhe envie as regras aprendidas através da Resposta de Regras. Após análisá-las, o usuário envia as regras aceitas na mensagem \texttt{New Rules} ao servidor. Este, por fim, envia uma notificação \texttt{Notify New Rules} ao controlador, informando-o sobre as novas regras.

\begin{figure}[h]
	\centering
	\caption{Sequência de trocas de mensagens na aceitação ou rejeição de um nó.}
  \includegraphics[width=0.7\textwidth]{imagens/mensagens_new_rule.png}
  \label{fig:mensagens_new_rule}  
\end{figure}
...
Por fim, note que a comunicação entre o servidor e o aplicativo envolve um servidor externo ao sistema, de um serviço chamado Firebase. O Firebase expõe uma API REST, permitindo que mensagens sejam enviadas a dispositivos móveis mediante a especificação de um token único para cada dispositivo. Efetivamente, a comunicação do servidor de aplicação com o aplicativo móvel é unidirecional através desse canal, justificando o porquê de não se esperarem respostas nas mensagens destinadas ao aplicativo.
...
da rede de sensores local foi realizada em \texttt{Javascript} utilizando o
ambiente \texttt{Node.js}, portanto, a implementação da biblioteca também foi
realizada utilizando-se estas tecnologias.

A biblioteca expõe funções para que o Controlador envie e receba mensagens do servidor seguindo o protocolo \texttt{Homecloud}, realizando de forma transparente ao usuário da biblioteca a autenticação junto ao servidor, a conexão do \texttt{WebSocket} e o enfileramento das mensagens quando não é possível efetuar o seu envio.
...
biblioteca foi disponibilizada na \texttt{npm}\footnote{Disponível em \url{https://www.npmjs.com/package/homecloud-controller}.}, que além de um gerenciador de pacotes para o \texttt{Node.js}, é um respositório público de bibliotecas.
